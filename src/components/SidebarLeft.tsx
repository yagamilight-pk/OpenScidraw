...
export interface ScientificSVG {
  id: string;
  name: string;
  category: string;
  svg: string;
...
const SCIENTIFIC_SVGS: ScientificSVG[] = [
  {
    id: 'cell',
...
export const SidebarLeft: React.FC = () => {
  ...
  const filtered = SCIENTIFIC_SVGS.filter(
    item => item.name.toLowerCase().includes(search.toLowerCase()) || 
            item.category.toLowerCase().includes(search.toLowerCase())
  );

  const handleDragStart = (e: React.DragEvent, item: ScientificSVG) => {
  ...
  return (
    <aside className="w-80 h-full bg-[#0f1623] border-r border-[#1e2d45] flex flex-col text-slate-200 overflow-hidden shrink-0">
      <div className="px-4 py-3.5 border-b border-[#1e2d45] shrink-0 bg-[#0b0f19]">
        <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400">Vector Workspace</h3>
      </div>
      <div className="px-3 py-2 shrink-0 border-b border-[#1e2d45]/50">
        <div className="relative">
          <input
            ...
            className="w-full bg-[#0b0f19] border border-[#1e2d45] rounded-lg pl-8 pr-3 py-1.5 text-xs text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-blue-500 transition-colors"
          />
